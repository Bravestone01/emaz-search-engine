...
export default function Pagination({ currentPage, query }: PaginationProps) {
  ...
  return (
    <nav className="flex flex-wrap items-center justify-center gap-9 text-[1.4rem] text-primary-text">
      <Link
        href={buildHref(query, previousPage)}
        aria-disabled={currentPage === 1}
        className={`rounded-full border border-primary-text px-9 py-4 transition-colors hover:bg-primary-secondary ${
      ...
      {Array.from({ length: TOTAL_PAGES }, (_, index) => {
        ...
        return (
          <Link
            ...
            className={`rounded-full border px-9 py-4 transition-colors ${
              isActive
